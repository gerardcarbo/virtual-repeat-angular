...
import { VirtualRepeatContainer, SCROLL_STATE } from './virtual-repeat-container';
...
export abstract class VirtualRepeatBase<T> {
  ...
  protected connect() {
    ...
    this._subscription.add(this._virtualRepeatContainer.scrollPosition
      .pipe(
        debounceTime(60),
        filter((scrollY) => {
          return scrollY == 0 || Math.abs(scrollY - this._scrollY) >= this._virtualRepeatContainer._rowHeight * this._guardItems / 2;
        })
      )
      .subscribe(
        (scrollY) => {
  ...
  protected findRequestedIndexesRange() {
    ...
    if (this._virtualRepeatContainer._autoHeightVariable) {
      ...
      this._virtualRepeatContainer.translateY = (this._firstRequestedItemIndex * this._virtualRepeatContainer._rowHeight);
      this.logger.log(`findPositionInRange: translateY: ${this._virtualRepeatContainer.translateY} rowHeight: ${this._virtualRepeatContainer._rowHeight}`);
  ...
  protected createViews(): Promise<ViewRef[]> {
    let promises = [];
    if (this._viewContainerRef.length > 0 && this._markedToBeRemovedCount < this._viewContainerRef.length) {
  ...
  protected applyStyles(index: number, view: EmbeddedViewRef<VirtualRepeatRow>) {
    let viewElement = view.rootNodes[0];
    ...
    if (this._virtualRepeatContainer._autoHeightVariable) {
      view.context.previousDisplay = viewElement.style.display;
      viewElement.style.display = 'none'; //will be shown when processing finished
  ...
  onProcessing(processing: boolean): any {
    if (processing === false) { //processing finished
  ...
  private dispatchLayout() {
    ...
    if (this._virtualRepeatContainer._autoHeight) {
      if (this._virtualRepeatContainer._autoHeightVariable) {
        ...
        for (let containerIndex = 0; containerIndex < this._viewContainerRef.length; containerIndex++) {
          let view = <EmbeddedViewRef<VirtualRepeatRow>>this._viewContainerRef.get(containerIndex);
          let viewElement: HTMLElement = view.rootNodes[0];
      ...
      //compute meanHeight
      for (let containerIndex = 0; containerIndex < this._viewContainerRef.length; containerIndex++) {
        let view = this._viewContainerRef.get(containerIndex) as EmbeddedViewRef<VirtualRepeatRow>;
        let viewElement: HTMLElement = view.rootNodes[0];

        let height = this.getElementHeight(viewElement);
        ...
        if (this._virtualRepeatContainer._autoHeightVariable) {
          if (view.context.previousDisplay != undefined) {
            ...
            if (this._virtualRepeatContainer._autoHeightVariableData.itemsCount < this._collectionLength) {
      ...
      if (this._virtualRepeatContainer._autoHeightVariable) {
        ...
        this._virtualRepeatContainer._rowHeight = this._virtualRepeatContainer._autoHeightVariableData.totalHeight / this._virtualRepeatContainer._autoHeightVariableData.itemsCount;
        ...
        if (this._fullScroll) {
          ...
          let translateY = this._virtualRepeatContainer.translateY +
            (this._virtualRepeatContainer.currentScrollState === SCROLL_STATE.SCROLLING_DOWN ? totalRemovedHeight : -totalAddedHeight);
          //check out of scroll
          let offset = this._scrollY - translateY;
        ...
        this.logger.log(`dispatchLayout: _autoHeightVariable rowHeight:${this._virtualRepeatContainer._rowHeight} 
